...
public final class TransformTrack {
    ...
    public static TransformTrack identity() { return constant(Transform.IDENTITY); }
    public List<Keyframe<Transform>> keyframes() { return keys; }
    ...
    public Transform sample(double tick) {
        ...
        Keyframe<Transform> a = keys.get(low), b = keys.get(high);
        double span = b.tick() - a.tick();
        double raw = span <= 0.0 ? 1.0 : (tick - a.tick()) / span;
        return Transform.lerp(a.value(), b.value(), a.interpolate(raw));
    }
}
